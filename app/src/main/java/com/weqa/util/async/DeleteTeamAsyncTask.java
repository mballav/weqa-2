...
public class DeleteTeamAsyncTask extends AsyncTask<Object, String, String> {

    public static interface UpdateUIAfterTeamDelete {
    ...
    public DeleteTeamAsyncTask(Retrofit retrofit, String logTAG, Activity activity) {
    ...
    protected String doInBackground(Object... params) {
        try {
            deleteTeam((DeleteTeamInput) params[0]);
        } catch (Exception e) {
    ...
    private void deleteTeam(DeleteTeamInput input) {
        AuthService service = retrofit.create(AuthService.class);
        Call<Void> call1 = service.deleteTeam(input);
        try {
...
/*            this.activity.runOnUiThread(new Runnable() {
    ...
    protected void onPostExecute(String status) {
        ...
        if (status.equals(STATUS_OK)) {
            ...
            Log.d(logTag, "Delete team response received!");

            this.activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ((DeleteTeamAsyncTask.UpdateUIAfterTeamDelete) activity).updateUIAfterTeamDelete();
                }
            });
        }
        else {
