...
public class ActivateUserAsyncTask extends AsyncTask<Object, String, String> {

    public static interface AfterActivate {
        public void afterActivate(ActivateUserResponse response);
    }
    ...
    private ActivateUserResponse response;

    public ActivateUserAsyncTask(Retrofit retrofit, String logTAG, Activity activity) {
    ...
    protected String doInBackground(Object... params) {
        try {
            activateUser((ActivateUserInput) params[0]);
        } catch (Exception e) {
    ...
    private void activateUser(ActivateUserInput input) {
        AuthService service = retrofit.create(AuthService.class);
        Call<ActivateUserResponse> call1 = service.activateUser(input);
        try {
    ...
    protected void onPostExecute(String status) {
        ...
        if (status.equals(STATUS_OK)) {
            ...
            this.activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ((ActivateUserAsyncTask.AfterActivate) activity).afterActivate(response);
                }
            });
        }
        else {
