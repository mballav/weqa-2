...
public class ResendCodeAsyncTask extends AsyncTask<Object, String, String> {

    public static interface AfterResendCode {
    ...
    public ResendCodeAsyncTask(Retrofit retrofit, String logTAG, Activity activity) {
    ...
    protected String doInBackground(Object... params) {
        try {
            resendCode((ResendCodeOrgInput) params[0]);
        } catch (Exception e) {
    ...
    private void resendCode(ResendCodeOrgInput input) {
        AuthService service = retrofit.create(AuthService.class);
        Call<Void> call1 = service.resendCode(input);
        try {
    ...
    protected void onPostExecute(String status) {
        ...
        if (status.equals(STATUS_OK)) {
            ...
            this.activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ((ResendCodeAsyncTask.AfterResendCode) activity).afterResendCode();
                }
            });
        }
        else {
