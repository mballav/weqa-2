...
/**
 * Created by Manish Ballav on 10/9/2017.
 */

public class RegistrationAsyncTask extends AsyncTask<Object, String, String> {

    public static interface AfterRegistration {
    ...
    public RegistrationAsyncTask(Retrofit retrofit, String logTAG, Activity activity) {
    ...
    protected String doInBackground(Object... params) {
        try {
            register((RegistrationInput) params[0]);
        } catch (Exception e) {
    ...
    private void register(RegistrationInput input) {
        AuthService service = retrofit.create(AuthService.class);
        Call<Void> call1 = service.register(input);
        try {
    ...
    protected void onPostExecute(String status) {
        ...
        if (status.equals(STATUS_OK)) {
            ...
            this.activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ((RegistrationAsyncTask.AfterRegistration) activity).afterRegistration();
                }
            });
        }
        else {
